...
<%-- 클라이언트가 페이지를 요청했을 때 한페이지를 담은 버퍼가 꽉차면 자동으로 서비스를 요청한 브라우저에 데이터를 전송해준다.
     ...
     autoFlush 제공 이유: 내 현재 웹페이지의 버퍼가 어느정도의 사이즈인지 알 수 없기 때문에 그 정보를 확인하기 위한 용도의 속성인 것이다.
                          !!8kb가 넘어가는지 확인을 하기 위한 용도인 것이다!! 
                          출력 버퍼를 너무 키워도 문제가 됨. 16kb로 설정을 해놓으면 16kb가 쌓일 때까지 전송이 되지 않음
                          즉, 적절한 버퍼크기를 체크하기 위함으로써 제공되는 속성이다.
                          입사하고 있는 회사에 따라서 어떤 버퍼로 잡아주고 있는지 알아보고 작업을 하면 됨
                          
	=> 사이트의 버퍼가 16kb이고 버퍼가 기본 버퍼사이즈라면 두 번으로 나뉘어 화면에 넘어갈 텐데 그렇다 보면 깜빡이는 느낌을 받을 수 있다.
	   그렇기 때문에 출력 버퍼의 깜빡이는 느낌을 받지 않기 위해서 출력버퍼 사이즈를 제대로 체크하는 것이 중요하다.
	
	프로젝트를 공유하고 싶다면 압축하여 공유할 수 있지만 사이즈가 너무 커짐 
	그렇기 때문에 웹에서는 주고받고자할 때(개발자끼리 완성한 프로젝트의 코드 내용들을 머지하는 상황) 머지된 코드는 배분해야한다!!
	=> WAR File을 통해 웹을 배포하도록 한다.(뒤에 가서 톰캣 9버전으로 다시 해볼 예정임)
	
	- 내보낼 프로젝트 오른쪽버튼 -> export -> WAR File을 통해 압축!! => 최소한의 소스를 가져갈 수 있게 해줌.
	- 집에서 압축에서 추가를 하려면 import를 하면됨
	(만약 파일에 에러가 뜬다면 설정에 들어가서 연결된 버전들을 다 확인하도록 한다. 원래는 환경에 맞게끔 포함이 되어짐) --%>
