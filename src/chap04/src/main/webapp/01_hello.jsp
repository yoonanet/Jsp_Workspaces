...
<body>
	...
	<%-- 내부적으로는 자바파일로 변환함, 어떻게 변환하는지 살펴볼 예정 
	     ...
	     java파일을 보면 위와 같은 코드를 한줄 한줄의 텍스트로 자바코드 변환한 것을 확인할 수 있다.
	     또한, 코드에 out은 출력하기 위한 것이고 지금 클라이언트 브라우저 화면에 연결이 되어져 있어서 데이터를 출력하라는 의미의 코드이다.
	     - 브라우저는 텍스트로 전달을 받고, 읽어서 브라우저화면을 구성하는 것이다.
	     
	     위 파일을 봤을 때가 자바코드로 작성된 것으로 작성을 하여 브라우저를 구성하기엔 복잡하고 번거롭다는 것을 알 수 있다. -> 복사해도 노가다임
	     따라서 필요성에 의해 jsp가 나타나게 된 것이다.
	            jsp는 자바파일로 만들어주는 것을 자바진영해서 해주기 때문에 자바일부의 문법 요소와 웹 표준을 통해 
	            좀 더 쉽게 웹사이트를 만들어서 서비스를 해줄 수 있는 것이다.
	     
	     => ***서블릿의 기술이 없어진 것이 아니라 백그라운드에서 다해주는 것이고,
	     	우리는 마치 웹표준으로만 작업을 하는 효과를 가져가는 것이다.
	     	
	     내장객체를 사용할 때 new를 안해줘도 됐었느냐. 그 이유가 위 파일에 설명이 되어지고 있음
	     HttpJspBase클래스 안에 메소드를 보면 매개변수에 response와 request로 전달받게끔 되어져 있음. -> 톰캣이 전달을 해주는 개념임
	     입력으로 객체를 전달받고, 메소드 안에서 코드가 작성이 되어지기 때문에 jsp페이지를 만들면 메서드 안에 작성이 되어지는 것이다.
	     그렇기 때문에 어느 위치에서든 내장객체를 문제없이 사용할 수 있다. -> 변환하는 과정에서 메소드 안에 변환되어 작업이 된다.
	     메소드를 호출할 때 입력으로 톰캣이 매개변수를 넣어주고 있기 때문이다!! 
	     
	     지역변수로 기본객체를 선언해주고 있음 =>  pageContext, session, config, out
	     지역변수로 선언한 이후, 필드에 메소드를 담아주고 있음.(메소드 안에 클래스를 정의를 해주고 있는 것이다. -> 익명클래스로써 객체를 반환하는 개념!!) --%>
	...
	<%-- [jsp의 내부적인 처리 과정]
	 ...
	 또한, 같은 페이지를 요청을 하게 되면 아까 전에 이미 한번 요청했던 jsp파일을 클래스 파일로 만들어놨기 때문에 
	      자바코드로 변환하는 과정을 거치지 않고 다이렉트로 클래스를 반환하면서 처리가 되어지고 있는 것이다.
	      퍼포먼스적으로 효율성을 극대화 시키는 것이다!!
